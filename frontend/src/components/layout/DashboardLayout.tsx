...
import {
  ...
  Target, Bell, Link2, ClipboardCheck, Calendar, DownloadCloud, Activity, KeyRound
} from "lucide-react";
...
const navItems = [
  ...
  { label: "SSO / SAML", icon: KeyRound, path: "/sso" },
  ...
  return (
    <div className="flex h-screen bg-[hsl(var(--background))] overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-[hsl(var(--border))]">
          ...
          <div className="min-w-0">
            <div className="font-bold text-white text-sm truncate">WorkforceAutomated</div>
            <div className="text-xs text-gray-500">AI Operating System</div>
          </div>
          <button
            className="ml-auto lg:hidden text-gray-400 hover:text-white p-1 min-w-[44px] min-h-[44px] flex items-center justify-center"
            onClick={closeSidebar}
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {active && !showBadge && <ChevronRight className="w-3 h-3 ml-auto flex-shrink-0" />}
              </Link>
            );
          })}
        </nav>
        ...
        <div className="px-3 py-4 border-t border-[hsl(var(--border))]">
          <div className="flex items-center gap-3 px-3 py-2 rounded-lg bg-surface-100">
            <div className="w-8 h-8 bg-brand-600/30 rounded-full flex items-center justify-center text-brand-400 font-semibold text-sm flex-shrink-0">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top bar */}
        <header className="flex items-center gap-2 sm:gap-4 px-4 sm:px-6 py-3 sm:py-4 border-b border-[hsl(var(--border))] bg-surface-DEFAULT/80 backdrop-blur-sm">
          <button
          ...
          <button
            className="btn-ghost p-2 relative min-w-[44px] min-h-[44px] flex items-center justify-center"
            onClick={() => { haptic("light"); navigate("/reviews"); }}
          ...
          <button
            ...
            onClick={() => { haptic("medium"); navigate("/agents/new"); }}
          >
